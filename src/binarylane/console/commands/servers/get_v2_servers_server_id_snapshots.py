...
from binarylane.api.servers.get_v2_servers_server_id_snapshots import sync_detailed
...
class Command(ListRunner):
    def response(self, status_code: int, received: Any) -> None:
        if status_code == 200 and isinstance(received, SnapshotsResponse):
            self._printer.print(received.snapshots, self._format)
        else:
...
""",
    ...
    def request(
        ...
        client: Client,
        request: object,
    ) -> Tuple[HTTPStatus, Union[None, ProblemDetails, SnapshotsResponse]]:
        assert isinstance(request, CommandRequest)

        # HTTPStatus.OK: SnapshotsResponse
        # HTTPStatus.NOT_FOUND: ProblemDetails
        # HTTPStatus.UNAUTHORIZED: Any
        page = 0
        per_page = 25
        has_next = True
        response: Optional[SnapshotsResponse] = None
        ...
        while has_next:
            ...
            assert isinstance(page_response.parsed, SnapshotsResponse)
