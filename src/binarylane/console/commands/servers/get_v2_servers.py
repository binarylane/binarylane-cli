...
class Command(ListRunner):
    def response(self, status_code: int, received: Any) -> None:
        if status_code == 200 and isinstance(received, ServersResponse):
            self._printer.print(received.servers, self._format)
        else:
            super().response(status_code, received)

    def lookup(self, value: str) -> Optional[int]:
        status_code, received = self.request(self._create_client(), CommandRequest())
        if status_code != 200:
            super().response(status_code, received)
