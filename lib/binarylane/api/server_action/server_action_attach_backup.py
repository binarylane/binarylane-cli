...
def _get_kwargs(
    ...
    *,
    ...
    url = "{}/v2/servers/{server_id}/actions#AttachBackup".format(client.base_url, server_id=server_id)
...
def sync_detailed(
    ...
    *,
    ...
    """Attach a Backup to a Server
     ...
     This is most often used to restore an individual file from a backup. It may also be used to boot the
    server from an ISO image.

    Args:
        server_id (int): The ID of the server on which the action should be performed.
        json_body (AttachBackup): Attach a Backup to a Server
