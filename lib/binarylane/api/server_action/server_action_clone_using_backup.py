...
def _get_kwargs(
    ...
    *,
    ...
    url = "{}/v2/servers/{server_id}/actions#CloneUsingBackup".format(client.base_url, server_id=server_id)
...
def sync_detailed(
    ...
    *,
    ...
    The action is 'performed' on the source server. The target server must have finished the initial
    building process: this will fail if the target server is does not have the status 'available'.
    **NB: This is a destructive operation on the target server and no further confirmation will be
    requested.**
